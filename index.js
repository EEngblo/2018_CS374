...
$( document ).ready(function() {
  ...
  $( "#budgetSlider" ).slider({
    ...
    slide: function( event, ui ) {
      $( "#budgetInputbox" ).val( Number(ui.value / 10000).toLocaleString('en') );
    },
  ...
  $( "#budgetInputbox" ).autocomplete({
    ...
    response: function(event, ui){
      if(ui.content.length == 1){
        ...
        $( "#budgetSlider").slider( "value", temp * 10000);
...
function budgetInputboxOnblur(){
  var temp = parseInt($( "#budgetInputbox" ).val().replace(/,/g, ''));
  if (isNaN(temp) || temp  < 65 || temp > 115){
    $( "#budgetInputbox" ).val(Number($("#budgetSlider").slider("value") / 10000).toLocaleString("en"));
    return;
  }else {
